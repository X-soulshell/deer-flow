...
下面这张 旅程图 把“工作区、产物与文件流”再压缩成一个可快速扫读的结构视图，便于先抓关键关系，再回到正文细节。

```mermaid
journey
    title 工作区、产物与文件流 的协作旅程
    section 起步
      运行时入口: 5: 用户, 平台
      扩展模块: 4: Lead Agent
    section 展开
...
- 上一篇：[78. 自定义 agent 配置体系](./78-custom-agent-configuration-system.md)
- 下一篇：[80. 观测、日志与评估](./80-observability-logging-and-evaluation.md)
